...
export default function BalancesPage() {
  ...
  function buildChatGPTUrl() {
    ...
    const expenseLines = expenses.map((e) => {
      ...
      const dateStr = e.expense_date ? ` (${e.expense_date})` : '';
      return `- ${e.description}${dateStr}: ${fmt(Number(e.amount), currency)} paid by ${payer}${splitParts ? ` — split: ${splitParts}` : ''}`;
    }).join('\n');
    const settlementLines = transactions.map(
...
${settlementLines || 'All settled — no transfers needed!'}

Trip link: ${shareLink}

Can you help me understand this expense split or answer any questions about it?`;
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-10 space-y-4">
        ...
        <div className="flex bg-zinc-100 rounded-xl p-1 gap-1">
          <button
            ...
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${tab === 'summary' ? 'bg-white text-black shadow-sm' : 'text-zinc-500'}`}
          >
            Summary
          </button>
          <button
            onClick={() => setTab('settle')}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${tab === 'settle' ? 'bg-white text-black shadow-sm' : 'text-zinc-500'}`}
          >
                ...
                <div key={stat.member.id} className="px-4 py-2.5 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <button
                      ...
                      className="w-4 h-4 rounded-full border border-zinc-200 text-zinc-400 hover:border-black hover:text-black transition-colors flex items-center justify-center text-[10px] font-bold leading-none flex-shrink-0"
                    >
                      ?
                    </button>
                  </div>
        ...
        {tab === 'settle' && (
          <div className="space-y-3">
            {/* ChatGPT button */}
            <button
              onClick={() => window.open(buildChatGPTUrl(), '_blank')}
              className="w-full flex items-center justify-center gap-2 bg-white border border-zinc-200 rounded-xl py-3 text-sm font-medium text-zinc-700 hover:border-zinc-400 transition-colors"
            >
                  ...
                  {transactions.map((t, i) => (
                    <div key={i} className="px-4 py-3 flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="flex-shrink-0 bg-[#25D366] text-white rounded-lg px-3 py-1.5 text-xs font-medium active:opacity-80"
                      >
                        Remind
                      </button>
                    </div>
